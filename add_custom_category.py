...
def get_category_type_id_by_name(conn, category_type_name):
    if is_conn(conn) and is_str(category_type_name):
        ...
        category_type_id = -1
        cursor.execute(
            "SELECT id,Typename FROM dbo.CategoriesTypes WHERE TypeName=?", category_type_name)
...
def add_category_type_by_name(conn, category_type_name):
    if is_conn(conn) and is_str(category_type_name):
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO dbo.CategoriesTypes (Typename) VALUES(?)", category_type_name)
        conn.commit()
        return True
    else:
        raise AttributeError(
...
def delete_categories_by_category_type_id(conn, category_type_id):
    if is_conn(conn) and is_int(category_type_id):
        cursor = conn.cursor()
        cursor.execute(
            "DELETE FROM dbo.Categories WHERE CategoryType=?", category_type_id)
        conn.commit()
    else:
        raise AttributeError(
...
def delete_categories_for_queries_by_category_type_id(conn, category_type_id):
    if is_conn(conn) and is_int(category_type_id):
        cursor = conn.cursor()
        cursor.execute(
            "DELETE FROM dbo.CategoryForQuery WHERE CategoryId IN (SELECT id FROM dbo.Categories WHERE CategoryType=?)",
            category_type_id)
        conn.commit()
    else:
        raise AttributeError(
...
def add_category(conn, category_name, category_type_id):
    if is_conn(conn) and is_str(category_name) and is_int(category_type_id):
        cursor = conn.cursor()
        cursor.execute("INSERT INTO dbo.Categories (CategoryName,CategoryType) VALUES(?,?)",
                       (category_name, category_type_id))
        conn.commit()
        return True
    else:
        raise AttributeError(
            "Connection object or Category Name or Category Type ID was not provided")


def get_category_id(conn, category_name, category_type_id):
    if is_conn(conn) and is_str(category_name) and is_int(category_type_id):
        cursor = conn.cursor()
        cursor.execute("SELECT Id FROM dbo.Categories WHERE CategoryName=? AND CategoryType=?",
                       (category_name, category_type_id))
...
def add_category_for_query(conn, category_id, query_id):
    if is_conn(conn) and is_int(category_id) and is_int(query_id):
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO dbo.CategoryForQuery (QueryId,CategoryId) VALUES(?,?)", (query_id, category_id))
        conn.commit()
        return True
    else:
        raise AttributeError(
...
def update_category_for_query(conn, category_id, query_id):
    if is_conn(conn) and is_int(category_id) and is_int(query_id):
        cursor = conn.cursor()
        try:
            cursor.execute("SELECT * FROM dbo.CategoryForQuery WHERE CategoryId=? AND QueryId=?",
                           (category_id, query_id))
            rows = cursor.fetchall()
            if len(rows[0]) == 0:
                cursor.execute("INSERT INTO dbo.CategoryForQuery (QueryId,CategoryId) VALUES(?,?)",
...
def get_categories_by_category_type_id_and_name(conn, category_name, category_type_id):
    if is_conn(conn) and is_int(category_type_id) and is_str(category_name):
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM dbo.Categories WHERE CategoryName=? AND CategoryType=?", category_name,
                       category_type_id)
        rows = cursor.fetchall()
        return rows
    else:
...
def get_categories_ids_by_category_type(conn, category_type_id):
    if is_conn(conn) and is_int(category_type_id):
        cursor = conn.cursor()
        cursor.execute(
            "SELECT [Id] FROM dbo.Categories where CategoryType=?", category_type_id)
        rows = cursor.fetchall()
        arr = array.array('i')
        for row in rows:
            category_id = row[0]
...
def main(args):
    ...
    if is_str(file_groups):
        ...
        if is_str(db_server) and \
            ...
            conn = connect_to_db(db_server, db_user, db_pwd, database)
            ...
            raise Exception(
                "db_server | db_user | db_pwd was not provided as an argument")
    else:
        raise Exception("file_groups was not provided as an argument")
